# ==============================================================================
# MÓDULO 0: CONFIGURACIÓN INICIAL Y LIBRERÍAS
# ==============================================================================
import sys
import subprocess
import importlib
...
# AUTO-INSTALADOR DE LIBRERÍAS (Eliminadas las dependencias de Google Sheets)
REQUIRED_PACKAGES = {
    'pandas': 'pandas',
    'numpy': 'numpy',
    'requests': 'requests',
...
try:
    ...
    print("\n✅ Módulo 0 ejecutado con éxito.")
except Exception as e:
    print(f"\n❌ Error crítico en la ejecución del Módulo 0. Detalles: {e}")

# ==============================================================================
# CONFIGURACIÓN DE RUTAS RELATIVAS Y NOMBRES DE ARCHIVO
# ==============================================================================
BASE_DIR = os.getcwd() # Ruta relativa al directorio de ejecución
...
try:
    ...
    print(f"✅ Calendario OK: {len(df_cal)} partidos procesados.")
    
    df_roster, ids_nuevos = actualizar_master_roster()
    print(f"✅ Master Roster OK: {len(df_roster)} jugadores guardados.")
...
def descargar_datos_partidos():
    if not os.path.exists(ARCHIVO_CALENDARIO):
        raise FileNotFoundError(f"❌ Error: No se encuentra el calendario en {ARCHIVO_CALENDARIO}.")
...
try:
    ...
    print(f"✅ Descarga Raw OK: {descargas} nuevos partidos extraídos.")
    print("\n✅ MÓDULO 2 EJECUTADO CORRECTAMENTE.")
except Exception as e:
    import traceback
    ...
    try:
        ...
        if os.path.exists(OUT_LINEUPS):
            ...
            if not tiempos_invalidos.empty:
                print(f"⚠️ AVISO LINEUPS (MINUTOS): {len(tiempos_invalidos)} equipos tienen sumas de tiempo inusuales.")
            else:
                print("✅ LINEUPS (MINUTOS): Todos los equipos suman 40/45/50 mins exactos en pista.")
            ...
            if not df_cal.empty:
                ...
                if 'HOME' in pts_lu and 'AWAY' in pts_lu:
                    ...
                    if not errores_lu.empty:
                        print(f"⚠️ AVISO LINEUPS (PUNTOS): {len(errores_lu)} partidos tienen puntos de quintetos que no cuadran con el Oficial.")
                    else:
                        print("✅ LINEUPS (PUNTOS): La suma de puntos de todos los quintetos cuadra con el Marcador Oficial.")
        ...
        if os.path.exists(OUT_PBP):
            ...
            if not incompletos.empty:
                print(f"⚠️ AVISO PBP (MINUTOS): {len(incompletos)} partidos no terminan en el segundo 00:00.")
            else:
                print("✅ PBP (MINUTOS): Todos los partidos registrados finalizan en el segundo 00:00.")
            ...
            if not df_cal.empty:
                ...
                if not errores_pbp.empty:
                    print(f"⚠️ AVISO PBP (PUNTOS): {len(errores_pbp)} partidos descuadran con el Marcador Oficial.")
                else:
                    print("✅ PBP (PUNTOS): Todos los marcadores máximos del Play-by-Play cuadran con el Calendario Oficial.")

    except Exception as e:
        print(f"⚠️ Fallo crítico en la auditoría de calidad: {e}")

try:
    procesados, fails = procesar_estadisticas_acumuladas()
    print(f"\n✅ MÓDULO 3 COMPLETADO.")
    print(f"📈 Partidos procesados en esta ejecución: {procesados} | Errores: {fails}")
    auditoria_calidad()
except Exception as e:
    import traceback
